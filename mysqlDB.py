...
#       print rows[0]['goodname']
    ...
    def replacePureData(self, tablename, columns, data):
        _prefix = 'REPLACE INTO ' + tablename
        _fields = ','.join(columns)
        _values = ','.join(['%s' for i in range(len(columns))]) 
        sql = _prefix + ' (' + _fields + ') VALUES (' + _values + ')'
        param = tuple(data)
        
#        print _prefix, _fields, _values, sql
        self.cur.execute(sql, param)

    def insertPureData(self, tablename, columns, data):
...
#        print _prefix, _fields, _values, sql
    ...
    def insertHasAttr(self, tablename, data):
        columns = data.keys()
        _prefix = 'INSERT INTO ' + tablename
        _fields = ','.join(columns)
        _values = ','.join(['%s' for i in range(len(columns))]) 
